...
export default function MapFields() {
  ...
  const clientdep = location.state.clientdeps;
  const [currentPage, setCurrentPage] = useState(1);
  const [entriesPerPage, setEntriesPerPage] = useState(5);
  const indexOfLastEntry = currentPage * entriesPerPage;
  const indexOfFirstEntry = indexOfLastEntry - entriesPerPage;
  const totalPages = Math.ceil(clientdep.length / entriesPerPage);
  const [query, setQuery] = useState("");
  const navigate = useNavigate();
  console.log("Inside mapField", clientdep, details);

  function handleMap(item) {
    navigate(
      "/departments/modules/mapdepartments/microapps/mapmicroapps/mapfields/mappingfields",
      { state: { details: details, item: item } }
    );
  }
  ...
  const filteredData = clientdep.filter((row) =>
                  ...
                  <div class="row">
                    <div class="col-md-5">
                      ...
                      >
                        ...
                        {Math.min(indexOfLastEntry, clientdep.length)} of{" "}
                        {clientdep.length} entries
                      </div>
                    </div>
